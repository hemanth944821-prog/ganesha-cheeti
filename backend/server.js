...
app.post('/api/change-password', async (req, res) => {
  ...
  if (!memberId || !newPassword) {
    return res.status(400).json({ success: false, message: 'Member ID and New Password are required' });
  }
  ...
  const mem = mockMembers.find(m => m.MemberID == memberId);
  if (mem) mem.Password = newPassword;
...
// --- MEMBERS ENDPOINTS (GET, POST, PUT, PATCH STATUS) ---
...
app.post('/api/members', async (req, res) => {
  ...
  try {
    ...
    if (pool) {
      ...
      const insertRes = await pool.request()
        ...
        .query(`INSERT INTO dbo.Members (MemberCode, Name_EN, Name_KN, Phone, Password, Role, Status) 
                VALUES (@code, @name_en, @name_kn, @phone, @password, @role, 'Active')`);
  ...
  const newMem = {
    MemberID: mockMembers.length + 1,
    MemberCode: `M#${(mockMembers.length + 1).toString().padStart(2, '0')}`,
    Name_EN: name_en,
    Name_KN: name_kn || name_en,
    Phone: phone,
  ...
  res.status(201).json({ success: true, member: newMem });
});

// Admin Toggle Deactivate / Reactivate Member (NO DELETION OF HISTORICAL DATA)
  ...
  try {
    ...
    if (pool) {
      await pool.request()
        ...
        .query(`INSERT INTO dbo.Expenses (Title_EN, Title_KN, Category, Amount, ExpenseDate) 
                VALUES (@title_en, @title_kn, @category, @amount, @expenseDate)`);
    }
  } catch (err) {
    console.error(err.message);
  }
...
app.delete('/api/expenses/:id', async (req, res) => {
  ...
  mockExpenses = mockExpenses.filter(e => e.ExpenseID !== id);
  res.json({ success: true, message: 'Expense deleted' });
});

// --- PAYOUTS / WINNERS ENDPOINTS ---
...
app.post('/api/payouts', async (req, res) => {
  ...
  try {
    ...
    if (pool) {
      await pool.request()
        ...
        .query(`INSERT INTO dbo.Payouts (MemberID, MonthYear, DrawDate, AmountWon, PaymentMethod) 
                VALUES (@memberId, @monthYear, @drawDate, @amountWon, @paymentMethod)`);
      return res.status(201).json({ success: true, message: 'Winner draw recorded' });
    }
  } catch (err) {
    console.error(err.message);
  }
  ...
  const newP = {
    PayoutID: mockPayouts.length + 1,
    MemberID: memberId,
    MonthYear: monthYear,
    AmountWon: amountWon || 2400,
    PaymentMethod: paymentMethod || 'UPI'
  };
  mockPayouts.unshift(newP);
...
app.post('/api/contributions', async (req, res) => {
  ...
  res.json({
    ...
    data: { memberId, amount: amount || 200, paymentMethod: paymentMethod || 'UPI' }
  });
});

app.listen(PORT, () => {
  console.log(`🚀 Ganesha Cheeti Express API running on http://localhost:${PORT}`);
});
